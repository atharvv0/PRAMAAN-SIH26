...
`GET /api/v1/health`, `POST/GET /api/v1/tasks`, and `POST /api/v1/tasks/{task_id}/run`
are implemented and tested — the `/run` endpoint wires the real
`services/orchestrator` Planner + Executor + ToolRegistry into the API and returns a
genuine execution result, including populated `evidence[]` for multimodal-shaped
intents (verified live end-to-end, not just unit-tested — see
`docs/api-contract.md` for a real captured response). Task storage is still an
in-memory dict, not Postgres. Everything else in `docs/api-contract.md` marked TODO
is a TODO — check the status column there before assuming an endpoint exists.
